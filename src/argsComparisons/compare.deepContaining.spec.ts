import { arrayContainingDeep, containingDeep, mapContainingDeep, setContainingDeep } from "../behaviours/constructs"
...
describe("compare.containingDeep", () => {
    ...
    it("should accept containing in deep maps", () => {
        ...
        const expected = mapContainingDeep(new Map<any, any>([
            ["c", new Map<any, any>([["d", 3]])]
        ]));
    ...
    it("should accept containing in deep sets", () => {
        const actual = new Set([
            1,
            new Set([2, 3, new Set([4, 5])])
        ]);

        const expected = setContainingDeep(
            new Set([new Set([new Set([5])])])
        )

        const expectedFalse = setContainingDeep(
            new Set([new Set([6])])
        )
        expect(compare(actual, expected)).toBe(true);
        expect(compare(actual, expectedFalse)).toBe(false);
